...
<div class="container-fluid">
	<form id='employee_frm'>
		...
		<div class="form-group">
			...
			<select class="custom-select browser-default select2" name="designation_id">
				<option value=""></option>
			<?php
			$pos = $conn->query("SELECT * from designation order by name asc");
			while($row=$pos->fetch_assoc()):
			?>
				<option class="opt" value="<?php echo $row['id'] ?>" data-did="<?php echo $row['department_id'] ?>" <?php echo isset($department_id) && $department_id == $row['department_id'] ? '' :"disabled" ?> <?php echo isset($designation_id) && $designation_id == $row['id'] ? " selected" : '' ?> ><?php echo $row['name'] ?></option>
			<?php endwhile; ?>
			</select>
		</div>
		<div class="form-group">
...
<script>
	$('[name="department_id"]').change(function(){
		var did = $(this).val()
		$('[name="designation_id"] .opt').each(function(){
