...
<?php
	$payroll=$conn->query("SELECT p.*,concat(e.lastname,', ',e.firstname) as ename,e.employee_no FROM payroll_items p inner join employee e on e.id = p.employee_id  where p.id=".$_GET['id']);
	foreach ($payroll->fetch_array() as $key => $value) {
		$$key = $value;
	}
	$pay = $conn->query("SELECT * FROM payroll where id = ".$payroll_id)->fetch_array();
	$pt = array(1=>"Monhtly",2=>"Weekly");
?>
				...
				<div class="card">
					...
					<div class="card-body">
						<ul class="list-group">
							...
							?>
							<li class="list-group-item d-flex justify-content-between align-items-center">
						   <?php echo $ded_arr[$val->did] ?> Deductions
					    <span class="badge badge-primary badge-pill"><?php echo number_format($val->amount,2) ?></span>
					  </li>
