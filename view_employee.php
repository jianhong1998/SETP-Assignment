...
<?php
$emp = $conn->query("SELECT e.*,d.name as dname,p.name as pname FROM employee e inner join department d on e.department_id = d.id inner join designation p on e.designation_id = p.id where e.id =".$_GET['id'])->fetch_array();
	foreach($emp as $k=>$v){
		$$k=$v;
	}
?>
...
<div class="contriner-fluid">
	<div class="col-md-12">
		...
		<p><b>Designation : <?php echo ucwords($pname) ?></b></p>
		<hr class="divider">
		<div class="row">
			<div class="col-md-6">
				<div class="card">
					...
					<div class="card-body">
						<ul class="list-group">
							...
							$t_arr = array(1=>"Monthly",2=>"Weekly",3=>"Once");
...
<script>
	$('#new_allowance').click(function(){
		uni_modal("New Allowace for <?php echo $employee_no.' - '.ucwords($lastname.", ".$firstname) ?>",'manage_employee_allowances.php?id=<?php echo $_GET['id'] ?>','mid-large')
	})
	$('#new_deduction').click(function(){
		uni_modal("New Deduction for <?php echo $employee_no.' - '.ucwords($lastname.", ".$firstname) ?>",'manage_employee_deductions.php?id=<?php echo $_GET['id'] ?>','mid-large')
	})
